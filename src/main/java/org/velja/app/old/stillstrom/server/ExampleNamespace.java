package org.velja.app.old.stillstrom.server;
...
import org.velja.app.old.stillstrom.server.methods.GenerateEventMethod;
import org.velja.app.old.stillstrom.server.methods.SqrtMethod;
import org.velja.app.old.stillstrom.server.types.CustomEnumType;
import org.velja.app.old.stillstrom.server.types.CustomStructType;
import org.velja.app.old.stillstrom.server.types.CustomUnionType;
...
public class ExampleNamespace extends ManagedNamespaceWithLifecycle {
    ...
    public static final String NAMESPACE_URI = "urn:eclipse:milo:stillstrom";
    ...
    private void createAndAddNodes() {
        // Create a "ChargerAvailable" folder and add it to the node manager
        NodeId folderNodeId = newNodeId("ChargerAvailable");
        UaFolderNode folderNode = createAndAddNode(folderNodeId);

        // Dodaj promenljivu ChargerAvailable u taj folder
        addChargerAvailableVariable(folderNode);
    }

    private void addChargerAvailableVariable(UaFolderNode parentNode) {
        NodeId nodeId = newNodeId("ChargerAvailable/Status");

        UaVariableNode node = UaVariableNode.builder(getNodeContext())
                .setNodeId(nodeId)
                .setAccessLevel(AccessLevel.READ_WRITE)
                .setUserAccessLevel(AccessLevel.READ_WRITE)
                .setBrowseName(newQualifiedName("ChargerAvailable"))
                .setDisplayName(LocalizedText.english("ChargerAvailable"))
                .setDataType(Identifiers.Boolean)
                .setTypeDefinition(Identifiers.BaseDataVariableType)
                .build();

        // Postavi inicijalnu vrednost
        node.setValue(new DataValue(new Variant(true), StatusCode.GOOD, DateTime.now()));

        // Dodaj promenljivu u node manager
        getNodeManager().addNode(node);

        if (getNodeManager().get(nodeId)!=null) {
            logger.info("ChargerAvailable successfully added to NodeManager.");
        } else {
            logger.error("Failed to add ChargerAvailable to NodeManager.");
        }

        // Poveži promenljivu sa folderom
        parentNode.addOrganizes(node);

        logger.info("ChargerAvailable variable created with NodeId: {}", nodeId);
    }
